...
procesarSTATA <- function(nombreArchivo){
                    ...
                    tablaSTATA = read_dta(paste("C:/Users/dante.ruiz/Documents/MARCEG/ENCHOR013/Base de Hogares_DTA/",nombreArchivo,".dta", sep = ""))
                    write.csv(tablaSTATA, file = paste("C:/Users/dante.ruiz/Documents/MARCEG/ENCHOR013/Bases_Hogares_CSV/",nombreArchivo,".csv", sep = ""))
                    return(tablaSTATA)                    
}
```
...
guardarTablaProcesada <- function(tabla){
                                          ...
                                          write.csv(tabla, file = paste("C:/Users/dante.ruiz/Documents/MARCEG/ENCHOR013/Bases_Hogares_Procesadas/",nombreArchivo,".csv", sep = ""))
                                                  }
```
...
* Catalogo_animales
```{r}
cat_gpo_animal <- read.csv("C:/Users/dante.ruiz/Documents/MARCEG/ENCHOR013/Catalogos/catalogo_grupo_animal.csv", colClasses = c("character","character"))
cat_animal <- read.csv("C:/Users/dante.ruiz/Documents/MARCEG/ENCHOR013/Catalogos/catalogo_animales.csv", colClasses = c("character","character"))
```
...
# Agregar columnas de individuos A, B, C, D, E
...
```{r}
ganaderia_household_labor_A  <- prod_ganade_ta %>%
                                    ...
                                    spread(s7c7gp04_ca,s7c7gp04a)
names(ganaderia_household_labor_A) <- c("cons","s7c7gp01","horas_A","minutos_A")
ganaderia_household_labor_A <- mutate(ganaderia_household_labor_A, min_en_hrs_A = minutos_A/60)

ganaderia_household_labor_B  <- prod_ganade_ta %>%
                                    
                                    filter(s7c7gp02 == 1) %>% # Trabajaron miembros del hogar en ganadería 1 sí / 2 no
  
                                    select(cons,          # número de cuestionario
                                           s7c7gp01,      # Grupo animal
                                           s7c7gp04b,     # Persona A: número horas o minutos al día le trabajó
                                           s7c7gp04_cb   # Persona A: código horas (1) o minutos (2)
                                           ) %>%
                                    spread(s7c7gp04_cb,s7c7gp04b)
names(ganaderia_household_labor_B) <- c("cons","s7c7gp01","horas_B","minutos_B")
ganaderia_household_labor_B <- mutate(ganaderia_household_labor_B, min_en_hrs_B = minutos_B/60)
