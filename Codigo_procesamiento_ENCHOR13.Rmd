...
cbind(sapply(ganaderia_produccion, class))
```
...
```{r}
ganaderia_produccion_resumen <- ganaderia_produccion %>% 
                                                group_by(cons, grupo, edo, mun, loc) %>% 
                                                summarise(total = sum(as.numeric(montoVentaTotal), na.rm = TRUE))
ganaderia_produccion_resumen <- left_join(ganaderia_produccion_resumen, cat_estado, by = c("edo" = "CVE_ENT"))
ganaderia_produccion_resumen <- left_join(ganaderia_produccion_resumen, cat_municipio, by = c("edo" = "CVE_ENT", "mun" = "CVE_MUN"))

guardarTablaProcesada(ganaderia_produccion_resumen)
```
